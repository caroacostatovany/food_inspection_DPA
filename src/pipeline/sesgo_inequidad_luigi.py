...
class TaskSesgoInequidadMetadata(CopyToTable):
    ingesta = luigi.Parameter(default="No", description="'No': si no quieres que corra ingesta. "
                                                        "'inicial': Para correr una ingesta inicial."
                                                        "'consecutiva': Para correr una ingesta consecutiva")
    ...
    table = "metadata.sesgo_inequidad"
    ...
    def requires(self):
        return [TaskSesgoInequidadUnitTesting(self.ingesta, self.fecha, self.threshold, self.algoritmo, self.metrica,
                                              self.kpi, self.permite_nulos)]

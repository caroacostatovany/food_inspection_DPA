...
from src.utils.constants import BUCKET_NAME, CREDENCIALES, NOMBRE_INICIAL, PATH_INICIAL, NOMBRE_CONSECUTIVO, PATH_CONSECUTIVO
...
class TaskAlmacenamiento(luigi.Task):
    ...
    def run(self):
        results = cargar_ingesta_local(self.file_to_upload)
        #outFile = open(self.output().path, 'wb')

        path_run = self.path_s3+"/"+self.file_to_upload
